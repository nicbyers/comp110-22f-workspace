"""An example of vectorized operations via operator overloading."""

from __future__ import annotations 
from typing import Union 


class StrArray:
    items: list[str]

    def __init__(self, items: list[str]):
        self.items = items 

    def __repr__(self) -> str:
        return f"StrArray({self.items})"

    def __add__(self, rhs: Union[str, StrArray]) -> StrArray:
        result: StrArray = StrArray([])
        # 1. Loop through each item in self's item list
        # 2. Concatenate rhs to the item value
        # 3. Append the resulting string to the result's item's list
        if isinstance(rhs, str):
            for item in self.items:
                item += rhs 
                result.items.append(item)
        else: 
            # 1. Loop through eact index(!!!!) of self's item list. 
            # 2. Concatenate the item at this index of self with the corresponding item at the same index of rhs's item list. 
            # 3. Append the concatenated string to the result's tem list. 
            assert len(self.items) == len(rhs.items)
            for i in range(len(self.items)):
                 result.items.append(self.items[i] + rhs.items[i])
        return result


a: StrArray = StrArray(["Armando", "Pete", "Leaky"])
b: StrArray = StrArray(["Bacot", "Nance", "Black"])
print(a)
print(a + "!")
print(a)
print(a + b)
print(b + "," + a + "!!!")